...
import type { PageResponse as JuzResponse } from "@/components/redux/api/quranSectionsApi";
...
interface JuzAyah extends Ayah {
  surah: SurahSummary;
}
...
const JuzView = ({ data }: JuzViewProps) => {
  // to get surah header
  ...
  const processedData: ProcessDataAyah[] = surahs.map((surah) => {
    const newData = (data.ayahs as JuzAyah[])
      .filter((ayah: JuzAyah) => ayah.surah.number === surah.number)
      .map(({ surah, ...rest }) => rest);
